...
| Adapter            |含义                            |
|--------------------|-------------------------------|
|ArrayAdapter\<T>    |用来绑定一个数组，支持范型操作      |
...
然后在Activity中初始化。
    ...
    public class MyListViewActivity extends Activity {
        private static final String[] strs = new String[]{ "first", "second", "third", "fourth", "fifth" }; // 定义一个String数组用来显示在ListView中
        ...
        protected void onCreate(Bundle savedInstanceState) {
            ...
            /* 为ListView设置Adapter绑定数据源 */
            lv.setAdapter(new ArrayAdapter<String>(this, android.R.layout.simple_list_item_1, strs));
        }
    }
    
代码非常的简单，运行效果如图4-29所示：   
![android_listview_demo_0](img/android_listview_demo_0.jpg "▲图4-29 ListView使用ArrayAdapter运行效果")   
...
2. 创建一个ArrayAdapter对象
3. 调用ListView.setAdapter(Adapter)方法绑定ArrayAdapter。

第二步有必要说一下，ArrayAdapter有多个构造函数，上例中使用的是最常用的一个。第一个参数是上下文Context。第二个参数为一个包含TextView，用来填充ListView每一行的布局资源文件ID。第三个参数为ListView要显示的数据内容。

其中第二个参数可以自定义一个layout,但是这个layout必须要有TextView控件。通常我们使用Android提供的资源，除了例子中所用的，常用的还有如下几种，可实现带RadioButton和CheckBox的ListView。（注意：在sdk22中，三种资源文件都使用CheckedTextView控件不同CheckMask实现）

1. 通过指定android.R.layout.simple_list_item_checked这个资源，实现带选择框的ListView。需要用setChoiceMode()方法设定选择是多选还是单选，否则将不能实现选择效果，运行效果如图4-30所示。实现代码如下：

        lv.setAdapter(new ArrayAdapter<String>(this, android.R.layout.simple_list_item_checked, strs));
        lv.setChoiceMode(ListView.CHOICE_MODE_MULTIPLE);
        
2. 通过指定android.R.layout.simple_list_item_multiple_choice这个资源实现带CheckBox的ListView。同样的，需要用setChoiceMode()设置单选或者多选，运行效果如图4-31所示。实现代码如下：

        lv.setAdapter(new ArrayAdapter<String>(this, android.R.layout.simple_list_item_multipe_choice, strs));
        lv.setChoiceMode(ListView.CHOICE_MODE_MULTIPLE);
        
3. 通过android.R.layout.simple_list_item_single_choice这个资源实现带RadioButton的ListView。这里要注意的是，这里并不是指定了单选。是单选还是多选要通过setChoiceMode()来指定，运行效果如图4-32所示。实现代码如下：

        lv.setAdapter(new ArrayAdapter<String>(this, android.R.layout.simple_list_item_single_choice, strs));
        lv.setChoiceMode(ListView.CHOICE_MODE_SINGLE);
        

![android_listview_demo_1](img/android_listview_demo_1.png "▲图4-30 带选择框的ListView")
![android_listview_demo_2](img/android_listview_demo_2.png "▲图4-31 带CheckBox的ListView")
![android_listview_demo_3](img/android_listview_demo_3.png "▲图4-32 带RadioButton的ListView")
...
很多时候需要在列表中展示一些除文字以外的东西，比如图片等。这时候可以用SimpleAdapter。SimpleAdapter的使用也非常简单，同时它的功能也非常强大。可以通过它自定义ListView中Item的内容，比如图片、多选框等。看一个例子，实现一个每一行都有一个ImageView和TextView的ListView。先看一下运行效果，如图4-34所示：

![android_listview_demo_4](img/android_listview_demo_4.png "▲图4-34 带图标的ListView")
...
使用SimpleAdapter的数据一般都是用HashMap构成的列表，列表的每一节对应ListView的每一行。通过SimpleAdapter的构造函数，讲HashMap的每个键的数据映射到布局文件中对应的控件上。这个布局文件一般根据自己需要自定义。

梳理一下使用SimpleAdapter的步骤：

1. 根据需要定义ListView每行所实现的布局
2. 定义一个HashMap构成的列表，将数据以键值对的方式存在里面
3. 构造SimpleAdapter对象
...
在ListView的使用中，有时候需要在里面加入按钮等控件，实现单独的操作。也就是说，这个ListView不再只是展示数据，也不仅仅是这一行要来处理用户的操作，而是里面的控件要获得用户的焦点。读者可以试试用SimpleAdapter添加一个按钮到ListView的条目中，会发现可以添加，但是却无法获得焦点，点击操作被ListView的Item覆盖。这时候最方便的方法就是灵活的适配器BaseAdapter了。
		...
		*/
		private class MyAdapter extends BaseAdapter {
			...
			public View getView(final int position, View convertView, ViewGroup parent) {
				...
				/*设置TextView显示的内容，即我们存放在动态数组中的数据*/				holder.title.setText(getDate().get(position).get("ItemTitle").toString());
...
![android_listview_demo_5](img/android_listview_demo_5.png "▲图4-36 使用BaseAdapter的ListView")
![android_listview_demo_6](img/android_listview_demo_6.png "▲图4-37 点击ListView条目和Button得到的输出")
...
![android_listview_demo_7](img/android_listview_demo_7.png "▲图4-38 滚动ListView输出的convertView的值")
